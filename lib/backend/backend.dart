...
Future maybeCreateUser(User user) async {
  final userRecord = AnnouncementsRecord.collection.doc(user.uid);
  final userExists = await userRecord.get().then((u) => u.exists);
  if (userExists) {
    currentUserDocument = await AnnouncementsRecord.getDocumentOnce(userRecord);
    return;
  }

  final userData = createAnnouncementsRecordData(
    email: user.email ??
  ...
  currentUserDocument =
      AnnouncementsRecord.getDocumentFromData(userData, userRecord);
}

Future updateUserDocument({String? email}) async {
  await currentUserDocument?.reference
      .update(createAnnouncementsRecordData(email: email));
}
